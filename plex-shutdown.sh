...
proc_num=$(ps -aux | grep -c 'Plex Transcoder') #number of plex processes right after startup
echo [$(date)] 'Initial Plex Processes:' $proc_num >> $log_path

while : ; do
	counter=0
	pending=false
	while [ $(ps -aux | grep -c 'Plex Transcoder') -le $proc_num ] ; do #if plex is not running
		sleep 1m
		((counter++))
		if [ $counter -lt 50 ] && [ $(ps -aux | grep -c 'Plex Transcoder') -gt $proc_num ] ; then #if plex start
			echo [$(date)] 'Counter:' $counter >> $log_path
			echo [$(date)] 'Plex Processes:' $(ps -aux | grep -c 'Plex Transcoder') >> $log_path
...
Type shutdown -c in a terminal to cancel."
	...
	if $pending && [ $(ps -aux | grep -c 'Plex Transcoder') -gt $proc_num ] ; then #if shutdown is pending and plex start
  		echo [$(date)] 'Counter:' $counter >> $log_path
		echo [$(date)] 'Plex Processes:' $(ps -aux | grep -c 'Plex Transcoder') >> $log_path
